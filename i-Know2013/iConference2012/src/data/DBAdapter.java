...
private final static String CreateWorkshopDes = "create table workshopDes (" +
	...
	//Tracks
	public long insertTrack(Track t)
	{
		ContentValues values = new ContentValues();
		values.put("ID", t.id);
		values.put("name", t.name);
		values.put("content", t.content);
		values.put("chair", t.chair);
		return mDb.insert("track", null, values);
	}
	
	public ArrayList<Track> getTracks()
	{
		ArrayList<Track> tList = new ArrayList<Track>();
		Cursor cursor = mDb.query("track", new String[] { "ID", "name","content","chair"}, null,null, null, null, null);
		...
		while (!cursor.isAfterLast()) {
			...
			t.id = cursor.getString(0);
			t.name = cursor.getString(1);
			t.content = cursor.getString(2);
			t.chair = cursor.getString(3);
			tList.add(t);
			cursor.moveToNext();
		}
		if (cursor != null && !cursor.isClosed()) {
	...
	public ArrayList<Paper> getPapersByTrackID(String trackID,String date) {

		ArrayList<Paper> papers = new ArrayList<Paper>();

		Cursor cursor = mDb.query("paper", new String[] { "ID", "title",
				"sessionName", "beginTime", "endTime", "date",
				"paperAbstract", "authors", "starred",
				"scheduled","room", "exactbeginTime", "exactendTime","type","trackID"}, "trackID='" + trackID + "' and date='"+date+"'", null,
				null, null, "exactbeginTime");
		cursor.moveToFirst();
		while (!cursor.isAfterLast()) {
			Paper p = new Paper();
			p.id = cursor.getString(0);
			p.title = cursor.getString(1);
//			p.belongToSessionName = cursor.getString(2);
//			p.bTime = cursor.getString(3);
//			p.eTime = cursor.getString(4);
			p.date = cursor.getString(5);
			p.paperAbstract = cursor.getString(6);
			p.authors = cursor.getString(7);
			p.starred = cursor.getString(8);
			p.scheduled = cursor.getString(9);
			//p.trackColor = cursor.getString(10);
			p.room = cursor.getString(10);
			p.exactbeginTime = cursor.getString(11);
			p.exactendTime = cursor.getString(12);
			p.type = cursor.getString(13);
//			p.trackID = cursor.getString(14);
			papers.add(p);
			cursor.moveToNext();
		}
